...
passport.use(
  new GoogleStrategy(
    {
      clientID:
        "759759818799-cb33i7mgtc2159cio27r0cgr4g571ao6.apps.googleusercontent.com",
...
exports.addUsersFromCSV = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(201).json({
      message: "Utilisateurs ajoutés avec succès depuis le fichier CSV.",
    });
  } catch (error) {
...
exports.createRoomRequest = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Room creation request sent successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
exports.handleRoomCreationRequest = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Room creation request handled successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
